...
class MVBIncomingText(models.Model):
    ...
    def compute_has_attachment(self):
        for em in self:
            attachments = self.env['ir.attachment'].search(
                [('res_model', '=', 'mvb.incoming.text'), ('res_id', '=', em.id)])
    ...
    def notification(self):
        ...
        rec = self.env['mvb.incoming.text'].browse(self.id)
        list_user = []
        for word in self.status_text:
            list_user.append(word.name.id)
            word.state = "thongbao"
        list_user_send = []
        for record in res:
            list_user_send.append(record.id)
            if record.id not in list_user:
                record_update_new = {
                ...
                rec.write({'status_text': [[0, 0, record_update_new]]})
        msg = 'Văn bản thông báo\nSố hiệu: ' + self.name + '\nTYND: ' + str(self.content_compendium)
        self.send_notify_users(list_user_send, msg, rec)
        print(list_user_send)
    ...
    def send_notify_users(self, list_user_send, msg, res):
        heading_msg = "Công ty Than Núi Hồng\n"
        result = []
        search_condition = [("user_id", "in", list_user_send)]
        one_signal_user_object = self.env['one_signal_notification.users_device_ids'].search(
            search_condition)
        for current_record in one_signal_user_object:
            result.append(current_record.name)
    ...
    def push_data_for_document(self):
        ...
        return {
            'effect': {
                'fadeout': 'slow',
                'message': 'Bạn đã lưu trữ vào phần quản lý văn bản',
                'type': 'rainbow_man',
            }
        }
    @api.model
    ...
    def open_project_project(self):
        if self:
            form_view_id = self.env.ref('project.edit_project').id
        return {
            'name': 'Quản lý giao việc',
            'type': 'ir.actions.act_window',
            'res_model': 'project.project',
            'context': "{'default_incoming_text_id': active_id}",
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': form_view_id,
            'target': 'new',
