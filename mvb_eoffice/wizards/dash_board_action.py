...
class DashBoardAction(models.TransientModel):
    ...
    def _count_incoming_solution(self):
        ...
        res = self.env['mvb.people.processing.text'].search_count(
            [('name', '=', self._uid), ('check_work', '=', False), ('state', '!=', 'theodoi'),('state', '!=', 'thongbao')])
        return res
    ...
    def _count_document_notification(self):
        res = self.env['mvb.people.processing.text'].search_count([('name', '=', self._uid), ('check_time_view', '=', False), ('state', '=', 'thongbao')])
        return res
        ...
        if res:
            if res.note_2:
                calendar_Html = '<h2 style="text-align: center; color: white; background-color: #5D8DA8; padding: 2px;">LỊCH CÔNG TÁC %s </br><small>(Từ %s đến %s)</small></h2>' % (
                    ...
                    res.date_to.strftime(
                        '%d/%m/%Y')) + '</br>' + res.calendar + '</br>' + '<p><b>Ghi chú:</b> %s</p>' % (res.note_2)
            else:
                calendar_Html = '<h2 style="text-align: center; color: white; background-color: #5D8DA8; padding: 2px;">LỊCH CÔNG TÁC %s </br><small>(Từ %s đến %s)</small></h2>' % (
                    res.name.upper(), res.date_from.strftime('%d/%m/%Y'),
                    res.date_to.strftime(
                        '%d/%m/%Y')) + '</br>' + res.calendar
        else:
            calendar_Html = '</br>'
    ...
    def _table_note(self):
        res = self.env['doimoi.total.calendar'].search([('state', '=', 'confirm')], limit=1, order='date_from DESC')
        if res.table_note:
            table_note_Html = '<h3>Lịch trực Bảo vệ và Lái Xe</h3>' + '</br>' + res.table_note
        else:
            table_note_Html = '</br>'
        return table_note_Html
    ...
    table_note = fields.Html(string="Lịch trực bảo vệ và lái xe", default=_table_note, readonly=True)
    ...
    def action_document_incoming(self):
        return {
            ...
            'domain': [('name', '=', self._uid), ('check_work', '=', False), ('state', '!=', 'theodoi'),('state', '!=', 'thongbao')],
            'view_type': 'form',
    ...
    def action_document_notification(self):
        ...
        return {
            ...
            'domain': [('name', '=', self._uid), ('check_time_view', '=', False), ('state', '=', 'thongbao')],
            'view_type': 'form',
            'res_model': 'mvb.people.processing.text',
            # 'view_id': False,
            'view_mode': 'tree',
