...
class MVBTextDirectionWizard(models.TransientModel):
    ...
    def check_user(self):
        if len(self.person_receiver_ids) == 0 and len(self.person_coordinator_handles_ids) == 0 and len(
                self.person_follow_ids) == 0:
    ...
    def action_solution_direction(self):
        ...
        res = self.sudo().env['mvb.incoming.text'].browse(
            self._context.get('active_id'))
        res.state = "pending"
        res_solu = self.env['mvb.solution.direction'].search(
            [('solution_id', '=', res.id), ('person_receiver_ids', '=', self._uid), ], limit=1)
        res_peo = self.env['mvb.people.processing.text'].search(
            [('text_ids', '=', res.id)])
        res_solution = self.env['mvb.solution.direction'].search(
            [('solution_id', '=', res.id)])
